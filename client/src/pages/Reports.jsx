...
//       url: "http://localhost:4000/api/reportsStore/" + id,
//       headers: {
//         Authorization: `Bearer ${user.token}`,
//       },
//     };
...
const Reports = () => {
  ...
  console.log("report is------>" ,reports);

  const fetchReports = async () => {
    
    try {
      const response = await axios.get(
        `http://localhost:4000/api/reportsStore?user_id=${user.user_id}`,
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const formData = new FormData();
      formData.append("file", selectedFile);
      formData.append("reportName", selectedFile.name); // Add the report name to the formData
      formData.append("resourceType", selectedFile.type); // Add the resource type to the formData
      formData.append("user_id", user.user_id); // Add the user_id to the formData
  ...
  const handleDelete = async (id, reportName) => {
    try {
      await axios.delete(`http://localhost:4000/api/reportsStore/${id}?user_id=${user.user_id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
    ...
    <Popover id="popover-basic">
      ...
      <Popover.Body>
        ...
        <Button
          ...
          className="mx-2"
          onClick={() => {
            handleDelete(deleteID, deletedFile);
            handleClosePopup();
          }}
        >
          Yes
        ...
        <div className="page-container">
          ...
          <div className="my-4" id="reports-page-container">
            <div id="reports-container">
              <div>
                <h3 className="charts-heading">
                  My Reports <FaFileAlt style={{ fontSize: "30px" }} />{" "}
                  {/* Placeholder icon */}
                </h3>
              </div>
              <h2>My Reports</h2>
